...
Implicit Q-Learning avoids explicitly estimating actions for Q-learning by using
expectile regression to estimate value functions and advantage-weighted regression
for policy extraction. This approach is simpler and more stable than CQL.

Key Insight: Instead of conservative penalties (CQL), IQL uses expectile regression
to learn an upper expectile of the value distribution, avoiding overestimation
while being simpler to implement.

Features:
- Expectile regression for value function learning
- Advantage-weighted behavioral cloning for policy
- No explicit action sampling required
- More stable than CQL with fewer hyperparameters

Example:
  # Generate offline dataset
  python iql_offline_rl.py --mode generate --dataset-path data/cartpole_medium.pkl

  # Train IQL on offline dataset
  python iql_offline_rl.py --mode train --dataset-path data/cartpole_medium.pkl

  # Compare IQL vs CQL vs BC
  python iql_offline_rl.py --mode compare --dataset-path data/cartpole_medium.pkl
...
class Config:
    # IQL specific
    expectile: float = 0.7  # Expectile for value function (τ)
    temperature: float = 3.0  # Temperature for advantage weighting (β)
...
def expectile_loss(diff: torch.Tensor, expectile: float) -> torch.Tensor:
    ...
    Asymmetric squared loss that puts more weight on positive or negative errors
    depending on the expectile parameter τ.

    Args:
        diff: Prediction error (pred - target)
        expectile: Expectile parameter τ ∈ [0, 1]
...
def train_iql(dataset: OfflineDataset, env_name: str, cfg: Config) -> QNetwork:
    """Train IQL on offline dataset."""
    console.print(f"\n[bold green]Training IQL[/bold green]")
    console.print(f"Dataset size: {len(dataset)}")
    console.print(f"Expectile τ: {cfg.expectile}")
    console.print(f"Temperature β: {cfg.temperature}")
    ...
    # IQL uses three networks: Q, V, and policy
    qnet = QNetwork(obs_dim, action_dim).to(cfg.device)
    vnet = ValueNetwork(obs_dim).to(cfg.device)
    policy = QNetwork(obs_dim, action_dim).to(cfg.device)  # Reuse Q-network architecture

    q_optimizer = optim.Adam(qnet.parameters(), lr=cfg.learning_rate)
    v_optimizer = optim.Adam(vnet.parameters(), lr=cfg.learning_rate)
    policy_optimizer = optim.Adam(policy.parameters(), lr=cfg.learning_rate)

    for update in track(range(cfg.num_updates), description="Training IQL"):
        ...
        # 1. Update Q-network (standard Bellman backup with V)
        q_values = qnet(state_batch).gather(1, action_batch.unsqueeze(1)).squeeze(1)

        with torch.no_grad():
            next_v = vnet(next_state_batch)
            target_q = reward_batch + cfg.gamma * next_v * (1 - done_batch)
        ...
        # 2. Update V-network (expectile regression)
        with torch.no_grad():
            # Get Q-value for the action taken in the dataset
            target_v = qnet(state_batch).gather(1, action_batch.unsqueeze(1)).squeeze(1)

        v_pred = vnet(state_batch)
        v_loss = expectile_loss(v_pred - target_v, cfg.expectile)
        ...
        with torch.no_grad():
            ...
            v_s = vnet(state_batch)
            advantage = q_sa - v_s

            # Advantage weighting with temperature
            weights = torch.exp(advantage / cfg.temperature)
            weights = torch.clamp(weights, max=100.0)  # Prevent explosion

        # Weighted behavioral cloning
        policy_logits = policy(state_batch)
        ...
        if update % 1000 == 0 and update > 0:
            console.log(
                f"Update {update}/{cfg.num_updates} | "
...
def train_cql(dataset: OfflineDataset, env_name: str, cfg: Config) -> QNetwork:
    """Train CQL for comparison (simplified version)."""
    console.print(f"\n[bold]Training CQL (for comparison)[/bold]\n")
...
def main():
    parser = argparse.ArgumentParser(description="IQL for Offline RL")
    parser.add_argument("--mode", type=str, default="compare",
                       choices=["generate", "train", "compare"],
                       help="Mode: generate dataset, train IQL, or compare methods")
    ...
    if args.mode == "generate":
        ...
        iql_policy = train_iql(dataset, args.env, cfg)
        evaluate_policy(iql_policy, args.env, cfg, "IQL")

    elif args.mode == "compare":
        # Compare IQL vs CQL vs BC
        console.print("[bold]Offline RL Comparison: IQL vs CQL vs BC[/bold]\n")
        ...
        iql_reward = evaluate_policy(iql_policy, args.env, cfg, "IQL")
        ...
        table.add_column("vs BC", style="yellow")

        table.add_row("Behavioral Cloning", f"{bc_reward:.2f}", "Baseline")

        cql_improvement = ((cql_reward - bc_reward) / abs(bc_reward)) * 100 if bc_reward != 0 else 0
        table.add_row("CQL (Conservative Q-Learning)", f"{cql_reward:.2f}", f"{cql_improvement:+.1f}%")

        iql_improvement = ((iql_reward - bc_reward) / abs(bc_reward)) * 100 if bc_reward != 0 else 0
        table.add_row("IQL (Implicit Q-Learning)", f"{iql_reward:.2f}", f"{iql_improvement:+.1f}%")

        console.print(table)

        # Determine winner
        best_method = "IQL" if iql_reward >= cql_reward else "CQL"
        console.print(f"\n[bold green]✓ {best_method} achieves best performance![/bold green]")
        console.print(f"[dim]IQL is typically simpler and more stable than CQL.[/dim]")
